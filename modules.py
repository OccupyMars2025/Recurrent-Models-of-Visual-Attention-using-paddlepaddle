...
class Retina:
    ...
    Returns:
        phi: a 2D tensor of shape (B, k*C*g*g). The
    ...
    def foveate(self, x, l):
        ...
        concatenated into a tensor of shape (B, k*C, g, g),
        then reshaped as (B, k*C*g*g).
        """
        phi = []
        size = self.g
    ...
    def denormalize(self, h, w, coords):
        """Convert coordinates in the range [-1, 1] to
        coordinates in the range [0, h) and [0, w)
        where `h` and `w` are the height and width
        of the image.
        """
        new_coords = 0.5 * (coords + 1.0)
        new_coords[:, 0] = new_coords[:, 0] * w
        new_coords[:, 1] = new_coords[:, 1] * h
        new_coords = new_coords.astype('int32')
        new_coords[:, 0] = paddle.clip(new_coords[:, 0], min=0, max=w-1)
        new_coords[:, 1] = paddle.clip(new_coords[:, 1], min=0, max=h-1)
...
class GlimpseNetwork(nn.Layer):
    ...
    Concretely, feed the output of the retina `phi` to
    ...
    def __init__(self, h_g, h_l, g, k, s, c):
        ...
        # glimpse layer
        self.fc1 = nn.Linear(k * c * g * g, h_g)
...
class CoreNetwork(nn.Layer):
    ...
    Args:
        ...
        g_t: a 2D tensor of shape (B, input_size). The glimpse
...
class ActionNetwork(nn.Layer):
    ...
    Concretely, feed the hidden state `h_t` through a fc
...
class LocationNetwork(nn.Layer):
    ...
    def forward(self, h_t):
        ...
        # reparametrization trick
        dist = Normal(mu, paddle.to_tensor(self.std, dtype='float32'))
        l_t = dist.sample(shape=[])
        l_t = l_t.detach()
        log_pi = dist.log_prob(l_t)
