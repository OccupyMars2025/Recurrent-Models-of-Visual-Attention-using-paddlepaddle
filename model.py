...
if __name__ == "__main__":
    ...
    h_g = 10
    h_l = hidden_size - h_g
    model = RecurrentAttention(g=8, k=6, s=1, c=1, h_g=h_g, h_l=h_l, std=1, hidden_size=hidden_size, num_classes=10)
    batch_size = 20
    x = paddle.uniform(shape=[batch_size, 1, 28, 28])
    l_t_prev = paddle.uniform(shape=[batch_size, 2], min=-1.0, max=1.0)
    h_t_prev = paddle.uniform(shape=[batch_size, hidden_size])
    last = True
    if last:
        h_t, l_t, b_t, log_probas, log_pi = model(x=x, l_t_prev=l_t_prev, h_t_prev=h_t_prev, last=last)
        print("h_t, l_t, b_t, log_probas, log_pi:")
        print(h_t, l_t, b_t, log_probas, log_pi, sep=2*'\n')
    else:
        h_t, l_t, b_t, log_pi = model(x=x, l_t_prev=l_t_prev, h_t_prev=h_t_prev, last=last)
        print("h_t, l_t, b_t, log_pi:")
        print(h_t, l_t, b_t, log_pi, sep=2*'\n')
